# AI Knowledge System

This vault is an AI-assisted knowledge system.

---

## Agent Entry Point (IMPORTANT)

If you are an AI agent:

1. Start by reading:
   → `[[06_SYSTEM_GUIDE/README.md]]`

2. Then read:
   → `[[06_SYSTEM_GUIDE/SYSTEM_RULES.md]]`

3. Do NOT scan the entire vault by default.

Use layered access:
- index → summary → full notes

---

## AI System Map (Full Links)

### System Prompts / Rules
- [[06_SYSTEM_GUIDE/README.md]]
- [[06_SYSTEM_GUIDE/SYSTEM_RULES.md]]
- [[06_SYSTEM_GUIDE/Design Principles.md]]
- [[06_SYSTEM_GUIDE/workflows.md]]
- [[06_SYSTEM_GUIDE/capabilities.md]]
- [[06_SYSTEM_GUIDE/Quick Commands.md]]
- [[06_SYSTEM_GUIDE/INIT_BOOTSTRAP_not_in_use.md]]
- [[06_SYSTEM_GUIDE/skills/organize_articles.md]]
- [[06_SYSTEM_GUIDE/skills/reorganize_article.md]]
- [[06_SYSTEM_GUIDE/skills/repair_links.md]]
- [[06_SYSTEM_GUIDE/tools/cleanup_orphan_summaries.sh]]
- [[06_SYSTEM_GUIDE/tools/validate_organize_articles.sh]]

### System Runtime Files
- [[01_INDEX/master_index.md]]
- [[01_INDEX/system_log.md]]

---

## Human Quick Use

- Human entry file: `[[README_HUMAN.md]]`
- Put new notes into `/00_RAW` (no status tag by default)
- New unprocessed notes may also appear in vault root (`/`) except `/README.md`
- Ask: “Process unprocessed notes”
- Ask: “What am I focusing on?”
- Ask: “Find notes about [topic]”

---

## If lost

Ask:
- “What can we do here?”
- “How does this system work?”

The system guide will help you.

---

## 下一篇
[[06_SYSTEM_GUIDE/README.md]]
